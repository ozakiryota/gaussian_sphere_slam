...
void WallEKFSLAM::CallbackDGaussianSphere(const sensor_msgs::PointCloud2ConstPtr &msg)
{
	...
	for(int i=0;i<list_lm_info.size();i++){
		...
		if(!list_lm_info[i].was_observed_in_this_scan){
			...
			const int threshold_count_match = 1000;
...
void WallEKFSLAM::RemoveUnavailableLM::Recover(Eigen::VectorXd& X, Eigen::MatrixXd& P, std::vector<LMInfo>& list_lm_info)
{
	if(unavailable_lm_indices.size()!=0){
		...
		for(size_t i=0;i<available_lm_indices.size();i++){
			...
			/*P-row-left*/
			tmp_P.block(
				size_robot_state_ + available_lm_indices[i]*size_wall_state_,
				0,
				size_wall_state_,
				size_robot_state_
			) = P.block(size_robot_state_ + i*size_wall_state_, 0, size_wall_state_, size_robot_state_);
			/*P-col-upper*/
			tmp_P.block(
				0,
				size_robot_state_ + available_lm_indices[i]*size_wall_state_,
				size_robot_state_,
				size_wall_state_
			) = P.block(0, size_robot_state_ + i*size_wall_state_, size_robot_state_, size_wall_state_);
			/*p-inside*/
			for(size_t j=0;j<available_lm_indices.size();j++){
				tmp_P.block(
					size_robot_state_ + available_lm_indices[i]*size_wall_state_,
					size_robot_state_ + available_lm_indices[j]*size_wall_state_,
					size_wall_state_,
					size_wall_state_
				) = P.block(size_robot_state_ + i*size_wall_state_, size_robot_state_ + j*size_wall_state_, size_wall_state_, size_wall_state_);
			}
		}
